...
## 更新摘要
**变更内容**
- 修复了Copilot服务中的sessionId类型转换错误，增强了对字符串和整数格式sessionId的支持
- 更新了前端API类型定义，包括assistantId字段、新增mode和context参数，以及修正了mentions字段位置
- 新增了智能、创意、严谨三种模式支持
- 增强了上下文参数支持
...
**更新** 本次更新修复了sessionId类型转换问题，增强了对字符串和整数格式sessionId的支持，并更新了前端API类型定义。
...
FE_TYPES["类型定义<br/>api.d.ts / copilot.d.ts"]
end
subgraph "业务 API 层"
...
ORCH_SCHEMAS["编排器Schema<br/>schemas.py"]
end
FE_API --> API_ROUTER
FE_STORE --> FE_API
FE_RENDER --> FE_STORE
...
ORCH_MAIN --> ORCH_CFG
ORCH_MAIN --> ORCH_SCHEMAS
```

**图表来源**
- [workspace/api/app/routers/copilot.py:1-137](file://workspace/api/app/routers/copilot.py#L1-L137)
- [workspace/api/app/services/copilot_service.py:1-125](file://workspace/api/app/services/copilot_service.py#L1-L125)
...
**更新** 增强了sessionId类型转换支持，现在可以处理字符串和整数格式的sessionId。

**章节来源**
- [workspace/api/app/routers/copilot.py:29-137](file://workspace/api/app/routers/copilot.py#L29-L137)
- [workspace/api/app/services/copilot_service.py:15-125](file://workspace/api/app/services/copilot_service.py#L15-L125)
...
**更新** 新增了mode和context参数支持，以及修正了mentions字段位置。
...
- 会话管理
  ...
  - **新增** 修复了sessionId类型转换错误，现在可以正确处理字符串和整数格式的sessionId

```mermaid
flowchart TD
Start(["进入 chat_stream"]) --> SaveUser["保存用户消息"]
SaveUser --> TypeCheck["检查 sessionId 类型转换"]
TypeCheck --> ParseInt{"尝试解析为整数"}
ParseInt --> |成功| UseInt["使用整数 sessionId"]
ParseInt --> |失败| CreateNew["创建新会话"]
UseInt --> Forward["POST /ai/chat (SSE)"]
CreateNew --> Forward
...
**更新** 增强了前端对新参数的支持，包括mode和context参数。
...
- [workspace/web/src/types/api.d.ts:143-150](file://workspace/web/src/types/api.d.ts#L143-L150)
...
- **新增** sessionId类型转换错误
  - 如果sessionId是字符串格式但服务端期望整数，服务端会自动尝试转换；如果转换失败，将创建新会话

**章节来源**
- [workspace/ai-orchestrator/app/main.py:97-120](file://workspace/ai-orchestrator/app/main.py#L97-L120)
- [workspace/api/app/services/copilot_service.py:90-125](file://workspace/api/app/services/copilot_service.py#L90-L125)
...
**更新** 最新版本增强了sessionId类型转换的健壮性，改进了前端API类型定义，支持更多交互模式和上下文参数。
...
- ChatRequest
  ...
  - **更新** 新增mode和context参数，修正mentions字段位置
...
### API类型定义
- CopilotRequest（前端类型）
  - 字段：assistantId、message、sessionId、mode、context、mentions
  - **更新** 包含所有新的参数字段
- ActionPreview
  - 字段：actionId、toolName、params、preview、expiresAt
- ActionExecute
  - 字段：actionId、toolName

**章节来源**
- [workspace/web/src/types/api.d.ts:143-163](file://workspace/web/src/types/api.d.ts#L143-L163)